...
@implementation AudioPlayer {
    ...
    BOOL _darwinEqEnabled;
    float _darwinEqPreampDb;
    float _darwinEqGainMultiplier;
    NSDictionary<NSString *, NSNumber *> *_darwinEqBandGains;
}
...
- (instancetype)initWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar playerId:(NSString*)idParam loadConfiguration:(NSDictionary *)loadConfiguration useLazyPreparation:(BOOL)useLazyPreparation {
    ...
    _darwinEqEnabled = NO;
    _darwinEqPreampDb = 0.0f;
    _darwinEqGainMultiplier = 1.0f;
    _darwinEqBandGains = @{};
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    @try {
        ...
        if ([@"load" isEqualToString:call.method]) {
            ...
            result(@{});
        } else if ([@"darwinSetEqualizer" isEqualToString:call.method]) {
            [self darwinSetEqualizer:request];
            result(@{});
        } else {
...
- (void)darwinSetEqualizer:(NSDictionary *)request {
    _darwinEqEnabled = [((NSNumber *)request[@"enabled"]) boolValue];
    _darwinEqPreampDb = [((NSNumber *)request[@"preampDb"]) floatValue];
    NSDictionary *rawBands = request[@"bandGainsDb"];
    if (rawBands && [rawBands isKindOfClass:[NSDictionary class]]) {
        NSMutableDictionary<NSString *, NSNumber *> *bands = [[NSMutableDictionary alloc] init];
        for (id key in rawBands) {
            id value = rawBands[key];
            if ([key isKindOfClass:[NSString class]] && [value isKindOfClass:[NSNumber class]]) {
                [bands setObject:(NSNumber *)value forKey:(NSString *)key];
            }
        }
        _darwinEqBandGains = bands;
    } else {
        _darwinEqBandGains = @{};
    }
    // dB to linear gain: 10^(dB/20)
    if (_darwinEqEnabled) {
        _darwinEqGainMultiplier = powf(10.0f, _darwinEqPreampDb / 20.0f);
        if (_darwinEqGainMultiplier < 0.0f) _darwinEqGainMultiplier = 0.0f;
        if (_darwinEqGainMultiplier > 2.0f) _darwinEqGainMultiplier = 2.0f;
    } else {
        _darwinEqGainMultiplier = 1.0f;
    }
    [self applyEffectiveVolume];
    // NOTE:
    // This hook persists EQ state per native player instance. Wiring this state
    // into the AVAudioEngine playback graph is implemented in the Darwin EQ
    // pipeline phase.
}

- (void)applyEffectiveVolume {
    if (!_player) return;
    float effective = _volume * _darwinEqGainMultiplier;
    if (effective < 0.0f) effective = 0.0f;
    if (effective > 2.0f) effective = 2.0f;
